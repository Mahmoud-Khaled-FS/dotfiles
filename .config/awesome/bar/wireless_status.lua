...
local function factory(args)
    ...
    function wirelessStatus:getInterface()
        awful.spawn.easy_async_with_shell("awk 'NR==3 {printf(\"%s\\n\", $1)}' /proc/net/wireless",
    ...
    wirelessStatus, wirelessStatus.timer = awful.widget.watch(
        {'awk', 'NR==3 {printf("%d-%.0f\\n", $2, $3*10/7)}', '/proc/net/wireless'}, wirelessStatus.timeout,
        function(self, stdout, stderr, exitreason, exitcode)
            ...
            self:settings()
        end, wirelessStatus -- base_widget (passed in callback function as first parameter)
    )
...
local wirelessStatus = wibox.widget {
    nil,
    factory(),
